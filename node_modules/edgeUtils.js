module.exports = {
    ...
    createEdgesFromREdges: function(REdges, neighbourLevel) {
        var edges = [];

        for (var i = 0; i < REdges.length; i++) {
            var source = REdges[i].value[0].value[0];
            var target = REdges[i].value[1].value[0];
            var weight = REdges[i].value[2].value[0];
            edges.push({
                data: {
                    id: source + target,
                    source: source,
    ...
    createEdgesFromREdgesFinal: function(REdges) {
        var edges = [];
