...
tryCatch(corMatrix <- readRDS(paste(filePath, fileName, sep="")),
           error = function(cond) {cat(format(toJSON(list(status = 1, message = "Failed to read the uploaded file. Please make sure that it is an RData file containing a matrix."), auto_unbox = TRUE))) ; write(paste("failed read", cond, sep=""), stderr()); file.remove(paste(filePath, fileName, sep="")); quit()}) 
